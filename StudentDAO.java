...
public class StudentDAO {
    ...
    public static Student findOne(int id) throws Exception {
        String sql = "SELECT * FROM student s LEFT JOIN department d ON s.departmentId = d.id Where s.id=?";
        try (Connection connection = DB.getConnection("student1");
             PreparedStatement statement = connection.prepareStatement(sql)) {
    ...
    public static void insert(String studentNumber, String name, int departmentId, int year) throws Exception{
    	String sql="INSERT INTO student(studentNumber, name, departmentId, year) Values(?, ?, ?, ?)";
    	try(Connection connection=DB.getConnection("student1");
    		PreparedStatement statement=connection.prepareStatement(sql);){
    ...
    public static Student selectByStudentNumber(String studentNumber) throws Exception{
    	String sql="Select * from studemt where studentNumber=?";
    	try(Connection connection=DB.getConnection("student1");
    		PreparedStatement statement=connection.prepareStatement(sql);){
    		statement.setString(1, studentNumber);
    		try(ResultSet result=statement.executeQuery();){
    			...
    			if(result.next()){
    				...
    				student.setDepartmentId(result.getInt("departmentId"));
